...
export function ContactForm() {
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      {/* Erreur globale */}
      {errors.global ? (
        <p className="rounded-xl bg-red-500/10 px-4 py-3 text-sm text-red-200 ring-1 ring-red-500/20">
          {errors.global}
        </p>
      ) : null}
      ...
      {/* Succès */}
      {state === "success" ? (
        <p className="rounded-xl bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200 ring-1 ring-emerald-500/20">
          {successMessage}
        </p>
      ) : null}
