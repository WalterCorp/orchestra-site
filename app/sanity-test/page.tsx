...
function portableTextToPlainText(content: unknown): string {
  ...
  return content
    .map((block) => {
      ...
      // Sanity Portable Text blocks are usually: { _type: "block", children: [{ text: "..." }, ...] }
    ...
    return (
      <main className="p-10 space-y-8">
        <header className="space-y-2">
          <h1 className="text-2xl font-bold">Sanity Test (Minimal Render)</h1>
          <p className="text-sm text-gray-600">
            Slug: <span className="font-mono">accueil</span>
          </p>
        </header>
        ...
        <section className="space-y-3">
          <h3 className="text-lg font-semibold">Debug (raw JSON)</h3>
